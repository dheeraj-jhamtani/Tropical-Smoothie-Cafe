(function (global, factory) {
	...
	var UNTRIMMABLE_TOKEN_TYPE = 'image';
	/**
	 * Trims the provided array of tokens to the specified length
	 *
	 * @param {Token[]} tokens
	 * @param {number} length
	 * @param {string} suffix
	...
	function trimTokensToLength(tokens, length, suffix) {
	  var availableLength = length;
	  var tokenIndex = 0;
	  var trimmedTokens = []; // Push tokens onto the list until we run out of availableLength
	  // Only the last token in this loop will have its content trimmed

	  while (tokenIndex < tokens.length && availableLength > 0) {
	    var token = tokens[tokenIndex];

	    if (token.type === UNTRIMMABLE_TOKEN_TYPE) {
	      trimmedTokens.push(token);
	    } else {
	      var _trimTokenToLength = trimTokenToLength(token, availableLength, suffix),
	          trimmedToken = _trimTokenToLength.trimmedToken,
	          remainingLength = _trimTokenToLength.remainingLength;

	      trimmedTokens.push(trimmedToken);
	      availableLength = remainingLength;
	    }

	    tokenIndex++;
	  } // We have finished trimming, so only push the closing tags
	  // needed to create valid HTML


	  var numIgnoredOpenTags = 0;

	  for (; tokenIndex < tokens.length; tokenIndex++) {
	    var _token = tokens[tokenIndex];
	    var isOpenTag = _token.nesting === 1;
	    var isClosingTag = _token.nesting === -1;

	    if (isClosingTag && numIgnoredOpenTags === 0) {
	      trimmedTokens.push(_token);
	    } else if (isOpenTag) {
	      numIgnoredOpenTags++;
	    } else if (isClosingTag && numIgnoredOpenTags > 0) {
	      numIgnoredOpenTags--;
	    }
	  }
	...
	/**
	 * Trims the content of the token.
	 * If the token has children, it will recusively trim all of the children.
	 * 
	 * @param {Token} token 
	 * @param {number} lengthToTrim 
	 * @param {string} suffix
	 * @returns 
	 */

	function trimTokenToLength(token, lengthToTrim, suffix) {
	  // It's necessary to mutate the token here, otherwise we lose prototype based methods like token.attrIndex
	  if (!token.children) {
	    var _remainingLength = lengthToTrim - token.content.length;

	    token.content = token.content.substr(0, lengthToTrim);

	    if (_remainingLength <= 0) {
	      token.content += suffix;
	    }
	  ...
	  var _trimTokensToLength = trimTokensToLength(token.children, lengthToTrim, suffix),
	      trimmedTokens = _trimTokensToLength.trimmedTokens,
	      remainingLength = _trimTokensToLength.remainingLength;

	  token.children = trimmedTokens;
	  return {
	    trimmedToken: token,
	...
	var YextRtfConverter = /*#__PURE__*/function () {
	  ...
	  /**
	   * Parses markdown into HTML that has been truncated to the given character count.
	   * 
	   * @param {string} markdown 
	   * @param {number} charCount
	   * @param {string} [suffix=...] The suffix to append to the end of the truncated HTML. This does not count towards the character count.
	   * @returns {string}
	   */
	  ;

	  _proto.toTruncatedHTML = function toTruncatedHTML(markdown, charCount, suffix) {
	    if (suffix === void 0) {
	      suffix = '...';
	    }
	    ...
	    var parsedTokens = this._md.parse(markdown, env);

	    var _trimTokensToLength = trimTokensToLength(parsedTokens, charCount, suffix),
	        trimmedTokens = _trimTokensToLength.trimmedTokens;

	    return this._md.renderer.render(trimmedTokens, this._md.options, env);
	  };
