...
function AppliedFiltersDisplay(props) {
    ...
    return ((0, jsx_runtime_1.jsxs)("div", Object.assign({ className: cssClasses.appliedFiltersContainer, "aria-label": 'Applied filters to current search' }, { children: [dedupedNlpFilterDisplaynames.map(displayName => renderNlpFilter(displayName, cssClasses)), dedupedRemovableFilters.map(f => {
...
function getDedupedRemovableFilters(filters) {
    ...
    for (const f of filters) {
        const preexistingDupe = dedupedFilters.find(d => (0, filterutils_1.isDuplicateFilter)(d.filter, f.filter));
        if (!preexistingDupe) {
